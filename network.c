...
#include "network.h"
#include "misc.h"   // ipmi_log()

/***************************************************************************
 *  Description:
 *      Receive a message sent by ipmi_send().  A uint32_t containing
 ...
 *      flags           See recv(2)
 *      timeout         useconds, passed to ipmi_recv
...
ssize_t ipmi_recv_munge(int msg_fd, char **payload, int flags, int timeout,
...

{
    ...
    char        incoming_msg[IPMI_MSG_LEN_MAX + 1];
    
    bytes_read = ipmi_recv(msg_fd, incoming_msg, IPMI_MSG_LEN_MAX + 1,
                           flags, timeout);
    
    if ( bytes_read == IPMI_RECV_FAILED )
    {
        ipmi_log("%s(): Error: ipmi_recv(fd = %d) failed: %s",
                __FUNCTION__, msg_fd, strerror(errno));
        return IPMI_RECV_FAILED;
    }
    else if ( bytes_read == IPMI_RECV_TIMEOUT )
    ...
    else
    {
        ...
        if ( munge_status != EMUNGE_SUCCESS )
        {
            ...
            ipmi_log("%s(): Error: munge_decode(fd = %d) failed.  %zd bytes, Error = %s\n",
        ...
        ipmi_send(msg_fd, 0, IPMI_MUNGE_CRED_VERIFIED_MSG);
        return payload_len;
    }
}
...
ssize_t ipmi_recv(int msg_fd, char *buff, size_t buff_len, int flags,
                  int timeout)

{
    ...
    if ( timeout != 0 )
    {
        ...
        if (poll_fd.revents & POLLHUP)
        {
            ...
            close(msg_fd);
            ipmi_log("%s(): Error: Lost connection to fd %d: HUP received.\n",
                    __FUNCTION__, msg_fd);
            return IPMI_RECV_FAILED;
        }
        ...
        if (poll_fd.revents & POLLERR)
        {
            ...
            ipmi_log("%s(): Error: Problem polling dispatchd: %s\n",
    ...
    {
        ipmi_log("%s(): Error: recv(fd = %d) returned -1: %s\n",
                __FUNCTION__, msg_fd, strerror(errno));
        return IPMI_RECV_FAILED;
    }
    else if ( bytes_read != sizeof(uint32_t) )
    {
        ipmi_log("%s(): Error: Read partial msg_len on fd = %d: %zd bytes.\n",
                __FUNCTION__, msg_fd,bytes_read);
    ...
    if ( msg_len > buff_len )
    {
        ipmi_log("%s(): Bug: msg_len (%" PRIi32 ") > buff_len - 1 (%zu) on fd = %d.\n",
...
/***************************************************************************
 *  Description:
 *      Open a socket connection to ipmi-gather
 *
 *  History: 
 *  Date        Name        Modification
...
{
    char                head_text_ip[IPMI_TEXT_IP_ADDRESS_MAX + 1];
    ...
    {
        ipmi_log("%s(): Error: socket() failed: %s",
                __FUNCTION__, strerror(errno));
        return -1;
    }
    ...
    if ( xt_resolve_hostname(hostname, head_text_ip,
                          IPMI_TEXT_IP_ADDRESS_MAX + 1) != XT_OK )
        exit(EX_OSERR);
    ...
    {
        ipmi_log("%s(): Error: connect() to %s (%s) failed: %s\n",
                __FUNCTION__, hostname, head_text_ip, strerror(errno));
        close(msg_fd);
        return -1;
    }
...
{
    char        *cred,
                incoming_msg[IPMI_MSG_LEN_MAX + 1];
    ssize_t     bytes;
    munge_err_t munge_status;
    
    if ( (munge_status = munge_encode(&cred, NULL, msg, strlen(msg))) != EMUNGE_SUCCESS )
    {
        ipmi_log("%s(): Error: munge_encode(fd = %d) failed: %s.\n",
                __FUNCTION__, msg_fd, munge_strerror(munge_status));
        // May be close(), ipmi_dispatchd_safe_close(), or ipmi_no_close()
        close_function(msg_fd);
        return IPMI_MUNGE_FAILED;
    }

    // ipmi_debug("%s(): Sending %zd bytes: %s...\n", __FUNCTION__, strlen(cred), cred);
    if ( ipmi_send(msg_fd, 0, cred) < 0 )
    {
        ipmi_log("%s(): Error: Failed to send credential to dispatchd",
                __FUNCTION__);
        // May be close(), ipmi_dispatchd_safe_close(), or ipmi_no_close()
        close_function(msg_fd);
        free(cred);
        return IPMI_SEND_FAILED;
    }
    free(cred);
    
    // ipmi_debug("%s(): Waiting for response.\n", __FUNCTION__);
    // Read acknowledgment
    bytes = ipmi_recv(msg_fd, incoming_msg, IPMI_MSG_LEN_MAX, 0, 0);
    if ( bytes == IPMI_RECV_FAILED )
    {
        ipmi_log("%s(): Error: ipmi_recv(fd = %d) failed.\n",
                __FUNCTION__, msg_fd);
        return IPMI_RECV_FAILED;
    }
    else if ( bytes == IPMI_RECV_TIMEOUT )
    {
        ipmi_log("%s(): Error: ipmi_recv(fd = %d) timeout.\n",
                __FUNCTION__,msg_fd);
        return IPMI_RECV_TIMEOUT;
    }
    if ( (bytes < 1) || (strcmp(incoming_msg, IPMI_MUNGE_CRED_VERIFIED_MSG) != 0) )
    {
        ipmi_log("%s(): Warning: Expected %s, got %zd bytes on fd = %d.\n",
                __FUNCTION__, IPMI_MUNGE_CRED_VERIFIED_MSG, bytes, msg_fd);
        return IPMI_RECV_FAILED;
    }
    // ipmi_debug("%s(): Done.\n", __FUNCTION__);
